...
def predict(
    ...
    if files:
        ...
        yield chatbot+[(inputs, "")], history, msg, all_token_counts
        with retrieve_proxy():
            llm_predictor = LLMPredictor(llm=OpenAIChat(temperature=0, model_name=ChatGPTModel))
    ...
    if stream:
        max_token = MODEL_SOFT_TOKEN_LIMIT[ChatGPTModel]["streaming"]
    else:
        max_token = MODEL_SOFT_TOKEN_LIMIT[ChatGPTModel]["all"]
